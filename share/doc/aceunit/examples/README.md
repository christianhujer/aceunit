...
- `hello_world`: These examples unit-test Hello, world!
  ...
  - The function under test has an outgoing side-effect by calling the `puts()` function.
    It thus has to replace `puts()` with a mock function so that the test can verify the side-effect,
